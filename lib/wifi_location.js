...
/**
 * Make BSSID of different form look like `00:AA:BB:CC:DD:EE`.
 *
 * @param {string} bssid BSSID in every form.
 * @return {string} Modified BSSID.
 */
const sanitize_bssid = bssid => {
    // remove all wxcepr hex digits
    bssid = bssid.toUpperCase().replace(/[^0-9A-F]/g, '');

    // result will be here
    let res = '';

    // get symbols
    const symbols = bssid.split('');

    // insert ':' every 2 symbols
    let i = 0;
...
const request3wifi = async bssid => {
    ...
    if (result.result && !Array.isArray(result.data)) {
        return {
            lat  : result.data[bssid][0].lat,
            lon  : result.data[bssid][0].lon,
            ssid : result.data[bssid][0].essid,
            range: null
        };
    } else {
        return null;
